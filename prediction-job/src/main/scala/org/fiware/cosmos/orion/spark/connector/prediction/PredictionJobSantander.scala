...
class PredictionJobSantander extends Serializable {
    ...
    def request (ent: EntityLD, MONGO_USERNAME: String, MONGO_PASSWORD: String, nombreCiudad: String): PredictionRequest  = {
        ...
        val variationStation: Double = variationStationsSantander(idVariationStation.toString).toString.toDouble
    ...
    def transform (rdd: RDD[PredictionRequest], spark: SparkSession): JavaRDD[Row] = {
      ...
      val df3 = df.withColumn("id_estacion", col("id_estacion").cast(IntegerType))
