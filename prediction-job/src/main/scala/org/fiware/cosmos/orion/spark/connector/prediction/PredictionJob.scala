  ...
  final val MONGO_PASSWORD = System.getenv("MONGO_PASSWORD")
    ...
    val processedDataStream = eventStream
      ...
      .map(ent => {
        ...
        val name = ent.attrs("type")("value").toString
        println(name)
        ...
        if (name == "ReqBarcelonaBikePrediction") {
        ...
        PredictionRequest(idStation, lastMeasure, tenHoursAgoMeasure, sixHoursAgoMeasure, nineHoursAgoMeasure, variationStation, weekday, hour, month, socketId, predictionId, name)
    ...
    val predictionDataStream = processedDataStream
      .transform(rdd => {
        ...
        if (df.select("ciudad").collect()(0) == "ReqBarcelonaBikePrediction") {
